function age_4_function!(variables::Mutable_Variables, prices::Mutable_Prices, parameters::NamedTuple, grids::NamedTuple)
    ...
    for c_i = 1:c_size, a_i = 1:a_size, h_i = 1:h_size, s_i = 1:s_size
        ...
        res = optimize(choices -> age_4_obj_function(choices, states, variables, prices, parameters, grids), choices_lb, choices_ub, choices_initial, Fminbox(NelderMead()))
...
function age_4_obj_function(choices::Vector{Float64}, states::Vector{Int64}, variables::Mutable_Variables, prices::Mutable_Prices, parameters::NamedTuple, grids::NamedTuple)
    """
    ...
    c_4 = budget - s_5
    s_5 = s_5 + s_min
    itp_V_5 = V_5_itp_function(s_5, h_5, states, variables, grids)
    return -utility_function(c_4, parameters) - β * itp_V_5
end

function V_5_itp_function(s_5::Real, h_5::Real, states::Vector{Int64}, variables::Mutable_Variables, grids::NamedTuple)
    """
    ...
    @unpack ϵ_size, ϵ_grid, a_Γ, a_size = grids
    s_i, h_i, a_i, c_i = states
    ...
    for a_k_i = 1:a_size
        for ϵ_i = 1:ϵ_size
            ...
            h_ind, h_wgt = locate_h_function(h_5_ϵ, grids)
