function age_7_function!(variables::Mutable_Variables, prices::Mutable_Prices, parameters::NamedTuple)
    ...
    for c_i = 1:c_size, a_i = 1:a_size, h_i = 1:h_size, s_i = 1:s_size, a_k_i = 1:a_size, h_k_i = 1:h_size
        ...
        res = optimize(choices -> age_7_obj_function(choices, states, variables, prices, parameters), choices_lb, choices_ub, choices_initial, Fminbox(NelderMead()))
...
function age_7_obj_function(choices::Vector{Float64}, states::Vector{Int64}, variables::Mutable_Variables, prices::Mutable_Prices, parameters::NamedTuple)
    ...
    construct the objective function at age 7
        ...
        states: (h_k_i, a_k_i, s_i, h_i, a_i, c_i)
    """
    @unpack h_grid, a_grid, s_grid, β, b, s_min, q, γ_2, d_2, ω_2, ζ = parameters
    ...
    c_7 = budget - s_8
    s_8 = s_8 + s_min
    itp_V_8 = V_8_itp_function(s_8, h_8, h_k_3, states, variables, parameters)
    return -q * utility_function(c_7, parameters) - β * itp_V_8
end

function V_8_itp_function(s_8::Real, h_8::Real, h_k_3::Real, states::Vector{Int64}, variables::Mutable_Variables, parameters::NamedTuple)
    """
    ...
    s_ind, s_wgt = locate_s_function(s_8, parameters)
    h_k_ind, h_k_wgt = locate_h_kid_function(h_k_3, parameters)
    for ϵ_i = 1:ϵ_size
        h_8_ϵ = ϵ_grid[ϵ_i, c_i] * h_8
        h_ind, h_wgt = locate_h_function(h_8_ϵ, parameters)
