...
! -----------------------------------------------------------
! 6: kid in primary school
! -----------------------------------------------------------
SUBROUTINE age6primary
USE BFGS_PQN
IMPLICIT NONE
...
!		ENDIF
		...
		CALL dfpmin_pqn(gue0,vt0,pl,pu,obj6,tol,maxfn,iflg)
!		pu0 = gue0
		pl0 = gue0
	...
	CALL MPI_ALLGATHER(v678split,nwp678,MPI_REAL8,v6fun,nwp678,MPI_REAL8,MPI_COMM_WORLD,ierr)
	CALL MPI_ALLGATHER(n67split ,nwp678,MPI_REAL8,n6fun,nwp678,MPI_REAL8,MPI_COMM_WORLD,ierr)
	CALL MPI_ALLGATHER(l12split ,nwp678,MPI_REAL8,l1fun,nwp678,MPI_REAL8,MPI_COMM_WORLD,ierr)
	CALL MPI_ALLGATHER(s78split ,nwp678,MPI_REAL8,s7fun,nwp678,MPI_REAL8,MPI_COMM_WORLD,ierr)

RETURN
END SUBROUTINE
! -----------------------------------------------------------
! -----------------------------------------------------------
REAL(dp) FUNCTION obj6(choices)
IMPLICIT NONE
	REAL(dp), INTENT(IN) :: choices(:)
	REAL(dp) :: hpx,nx,sx
	REAL(dp) :: cx,v7x,budget
...
!	hkx = hkx**(1d0/phik)
	...
	budget = atinc(wp*(1d0-nx-lx)-mx,ss,6) +ss -smin
	...
	v7x = ipolatev7(hpx,hkwgts,hki1,swgts,si1)

	obj6 = -qq*u(cx) -bta*v7x

RETURN
END FUNCTION
! -----------------------------------------------------------
REAL(dp) FUNCTION ipolatev7(hpx,hkwgts,hki1,swgts,si1)
IMPLICIT NONE
	...
	DO ei=1,en
		...
		temp(ei,1) = DOT_PRODUCT(swgts,v7fun(si1:si1+1,hi1,hki1,ai,aii,ci))
		temp(ei,2) = DOT_PRODUCT(swgts,v7fun(si1:si1+1,hi1+1,hki1,ai,aii,ci))
		temp(ei,1) = DOT_PRODUCT(hwgts,temp(ei,1:2))

		temp(ei,2) = DOT_PRODUCT(swgts,v7fun(si1:si1+1,hi1,hki1+1,ai,aii,ci))
		temp(ei,3) = DOT_PRODUCT(swgts,v7fun(si1:si1+1,hi1+1,hki1+1,ai,aii,ci))
		temp(ei,2) = DOT_PRODUCT(hwgts,temp(ei,2:3))
	ENDDO
	temk(1) = SUM(temp(:,1))/en
	temk(2) = SUM(temp(:,2))/en

	ipolatev7 = DOT_PRODUCT(hkwgts,temk)
